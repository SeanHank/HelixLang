...
cobra, rdkit, ESM, native accel) is unavailable, the run fails with an actionable
error naming the missing component and the `pip install helixlang[<extra>]` fix,
unless you explicitly opt into reduced fidelity with a capability flag.
...
| SHA256 golden verification | **85/85** golden files (84 pass, 1 skip; `11_performance_comparison` is performance-only, no hash) |
...
Full technical documentation in [`doc/`](https://github.com/SeanHank/HelixLang/tree/main/doc) (43 files, 25,000+ lines):
...
- **3884 test cases**(all passing, 81% coverage)
