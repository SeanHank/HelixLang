...
from helixlang.human.clinical_output import ClinicalLabModel, ClinicalLabs, VitalsModel
from helixlang.human.ddi import DDIModel, assess_additive_toxicity
...
from helixlang.human.drug import IV, IV_INFUSION, Drug
from helixlang.human.genotype import CORE_CYP_ENZYMES, GenotypeProfile, create_default_genotype
from helixlang.human.organ_crosstalk import apply_crosstalk, create_crosstalk
...
try:
    from helixlang.human.gem_human import HumanGEMLoader
except ImportError:  # pragma: no cover
    HumanGEMLoader = None  # type: ignore[assignment,misc]

try:
    from helixlang.human.endocrine import EndocrineSystem
    from helixlang.human.hematology_model import HematologySystem
    from helixlang.human.immune import CRPDriver, InnateImmuneModel
    from helixlang.human.qsp_binding import QSPBindingSystem
    from helixlang.human.renal_model import RenalFunctionModel
except ImportError:  # pragma: no cover
...
def _import_doc32() -> None:
    ...
    if _stochastic_ode is None:
        try:
            from helixlang.human.stochastic_ode import SDEConfig
    ...
    if _tissue_gem is None:
        try:
            from helixlang.human.tissue_gem import GEMDecomposer, OrganGEMCoupler
            _tissue_gem = (GEMDecomposer, OrganGEMCoupler)
        except ImportError:
            pass
    if _reduced_order is None:
        try:
            from helixlang.human.reduced_order_organ import PODModeGenerator
            _reduced_order = PODModeGenerator
        except ImportError:
            pass
    if _pharmacogenomic_ae is None:
        try:
            from helixlang.human.pharmacogenomic_ae import GenotypeAEPredictor
...
class VirtualPatient:
    ...
    def _build_endocrine(self) -> EndocrineSystem | None:
        ...
        from helixlang.human.endocrine import create_endocrine
        diabetes_sev = 0.0
        addison_sev = 0.0
        cushing_sev = 0.0
        stress_level = 0.0
    ...
    def _build_qsp_binding(self) -> QSPBindingSystem | None:
        ...
        from helixlang.human.qsp_binding import create_qsp_binding
    ...
    def _init_immune_if_needed(self) -> None:
        ...
        if self._immune is not None:
            return
        from helixlang.human.immune import create_immune_model
    ...
    def _init_hematology_renal_if_needed(self) -> None:
        ...
        if self._hematology is not None:
            return
        try:
            from helixlang.human.hematology_model import create_hematology_system
            from helixlang.human.renal_model import create_renal_model
    ...
    def run(self) -> VirtualPatientResult:
        ...
        for step_i in range(n_steps + 1):
            ...
            if self._disease_model is not None and self.config.disease is not None:
                ...
                from helixlang.human.disease_progression import ClinicalLabs as ProgLabs
