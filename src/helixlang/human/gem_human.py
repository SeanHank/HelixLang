...
try:
    from helixlang.metabolism import ECOLI_CORE_MODEL, MetabolicModel, Reaction
except ImportError:
...
try:
    from helixlang.human.physiology import TISSUE_PROFILES
except ImportError:
...
def create_human_biomass_reaction() -> Any:
    ...
    if Reaction is None:
        raise ImportError("helixlang.metabolism required")
...
class HumanGEMLoader:
    ...
    def load_from_sbml(self, path: str) -> Any:
        ...
        Uses CobraPy if available, otherwise falls back to core model.
        """
        try:
