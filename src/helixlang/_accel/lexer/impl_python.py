"""Pure-Python reference lexer (doc/06 §19, compiler+VM C-only mandate).

A faithful scalar port of the lexer that previously lived in
``helixlang.core.lexer``.  It shares the module contract the native backend
must match — ``tokenize(source, is_annotation_keyword) -> list[Token]`` — and
exists only as the **non-selectable** reference used to verify the C kernel
(``impl_cext``) is byte-for-byte equivalent.  The compiler+VM core refuses to
select it (``_NATIVE_ONLY_PACKAGES``, doc/03 §6.5).
"""
from __future__ import annotations

from helixlang.core.errors import LexError
from helixlang.core.lexer import Token


#: Default predicate when none is injected: every bare ``#ident`` is a gene-ID
#: marker, exactly what the C kernel does when ``is_annotation_keyword is None``.
def _no_keywords(name: str) -> bool:
    return False


class Lexer:
    """Dual-mode scanner (reference algorithm, mirrors ``impl_cext.c``).

    ``keyword_check`` is the ``#ident`` -> annotation-keyword predicate (the
    core ``ANNOTATION_KEYWORDS`` set plus the plugin grammar registry) owned by
    ``helixlang.core.lexer`` and injected by :func:`tokenize`.
    """

    BASES = set("ACGTacgt")

    def __init__(self, source: str, keyword_check=_no_keywords):
        # Preserves original case (annotation field names/values are
        # case-sensitive); DNA bases are uppercased inside _scan_dna.
        self.src = source
        self.pos = 0
        self.line = 1
        self.col = 1
        ...
        self.keyword_check = keyword_check
...
def tokenize(source: str, is_annotation_keyword=None):
    """Reference ``tokenize`` — same contract as ``impl_cext``.

    Returns the list of :class:`helixlang.core.lexer.Token` instances the
    native kernel must reproduce exactly (kind, value, line, col and, for
    CODON tokens, the running ``codon_index``).
    """
    check = is_annotation_keyword or _no_keywords
    return list(Lexer(source, keyword_check=check).tokens())
