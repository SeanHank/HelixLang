...
def _predict_motifs(
    ...
    try:
        ...
        if set(first_line.upper()) <= {"A", "C", "G", "T", "N", " "}:
            is_nucleotide = True
    except Exception:  # SILENTBENIGN - unreadable genome -> default plain
        pass
    ...
    except (OSError, IndexError):  # SILENTBENIGN - missing/empty FASTA -> no edges
        pass
...
def _extract_upstream_sequences(
    ...
    except Exception:  # SILENTBENIGN - unreadable FASTA -> empty upstream
        pass
