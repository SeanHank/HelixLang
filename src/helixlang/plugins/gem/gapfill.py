...
from helixlang.plugins.runtime.metabolism import FluxBalanceAnalysis, MetabolicModel, Reaction
...
def gapfill(
    ...
    if model.biomass_reaction and model.biomass_reaction in model.reactions:
        ...
        try:
            ...
            if fluxes.get(model.biomass_reaction, 0.0) > 1e-9:
                ...
                return result
        except Exception:  # SILENTBENIGN - continue to LP gap-fill pass
            pass
    ...
    if model.biomass_reaction and model.biomass_reaction in model.reactions:
        try:
            ...
            if fluxes.get(model.biomass_reaction, 0.0) > 1e-9:
                result.biomass_blocked = False
        except Exception:  # SILENTBENIGN - keep biomass_blocked default
            pass
    return result
