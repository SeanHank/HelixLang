...
def goldman_decode(oligos: Iterable[GoldmanOligo],
    ...
    for oligo in oligo_list:
        ...
        except (ValueError, KeyError):  # SILENTBENIGN - try other orientation
            pass
        if seg_idx is None:
            # try RC
            try:
            ...
            except (ValueError, KeyError):  # SILENTBENIGN - explicit oligo fallback
                pass
        if seg_idx is None:
...
def sequence_dna(dna: str,
    ...
    from helixlang.plugins.runtime.bio_data import SEQUENCING_PLATFORM_ERROR_RATES
...
def decay_dna(dna: str, years: float, temperature_c: float = 13.1,
    ...
    if rng is None:
        rng = random.Random(0)
    from helixlang.plugins.runtime.bio_data import dna_survival_fraction
