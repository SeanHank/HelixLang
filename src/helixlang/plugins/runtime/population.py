...
.. note::
   ...
   **µm²/s** coefficient (converted internally via
   :func:`helixlang.core.units.diffusion_to_lattice` at the declared 10 µm
...
from helixlang.plugins.runtime.flow import FlowField, FlowField3D
...
if TYPE_CHECKING:
    from helixlang.plugins.apps.genome_scale import GenomeColony, GenomeSpec
...
class PopulationConfig:
    ...
    Args:
        ...
        program: HelixLang :class:`~helixlang.core.ast_nodes.Program` run
        ...
        dfba_enabled: per-cell dynamic-FBA metabolism (Phase 5,
            ...
            :class:`~helixlang.plugins.runtime.metabolism.DynamicFluxBalance` that reads
        ...
        genome: genome-scale template (doc/18-programmable-cell-population-simulation.md Design 5).  When set, every
            ...
            `~helixlang.plugins.runtime.sparse_grn.SparseGRN` call per tick, instead of
...
class PopulationCell:
    ...
    :class:`~helixlang.plugins.runtime.grn.GRN` and suspended bytecode VM state
...
def divide_cell(
    ...
    if config.cell_shape == "rod" and cell.body is not None:
        from helixlang.plugins.runtime.cell_body import divide_rod
...
class CellPopulation:
    ...
    Wright-Fisher evolutionary population ``EvolutionaryPopulation`` in ``helixlang.plugins.runtime.evolution``
    ...
    def __init__(
        ...
        if config.genome is not None:
            from helixlang.plugins.apps.genome_scale import GenomeColony
            if not _HAS_NUMPY:
                raise ValueError(
    ...
    def _diffuse(self, config: PopulationConfig) -> list[list[float]]:
        ...
        on-lattice form via :func:`helixlang.core.units.diffusion_to_lattice`
    ...
    def _step_lbm(self) -> None:
        ...
        the refreshed :class:`~helixlang.plugins.runtime.flow.FlowField` is attached to
    ...
    def _step_dfba_metabolism(self) -> tuple[list[PopulationCell], int]:
        ...
        Each alive cell owns a :class:`~helixlang.plugins.runtime.metabolism.
...
class CellPopulation3D(CellPopulation):
    ...
    - :meth:`to_lsystem3d` exports the colony occupancy as an
      :class:`helixlang.plugins.runtime.morphology_3d.LSystem3D` geometry for
    ...
    def _step_lbm(self) -> None:
        ...
        A :class:`~helixlang.plugins.apps.lattice_boltzmann_3d.LatticeBoltzmann3D`
        (D3Q19) solver is advanced over ``config.flow_substeps`` ticks
        with the alive cells as no-slip obstacles and the resulting
        :class:`~helixlang.plugins.runtime.flow.FlowField3D` attached to
        ``config.flow3d`` (3D drift + environment advection).  A plain
        2D :class:`~helixlang.plugins.apps.lattice_boltzmann.LatticeBoltzmann`
    ...
    def to_lsystem3d(self, step: float = 1.0,
        ...
        :class:`helixlang.plugins.runtime.morphology_3d.LSystem3D` for drawing/rendering
...
# ``CellPopulation`` to distinguish it from ``helixlang.plugins.runtime.evolution.EvolutionaryPopulation``.
