...
def _load_ec_map(ec_map_path: str | None) -> dict[str, str]:
    ...
    except Exception:  # SILENTBENIGN - malformed optional EC map -> {}
        pass
    return {}
...
def _stage_e_community(
    ...
    try:
        ...
        except Exception:  # SILENTBENIGN - tolerant flux-summary cleanup
            pass
        org = OrganismModel(
...
def _run_dfba_simulation(model: Any, config: PipelineConfig) -> dict[str, Any]:
    ...
    try:
        from helixlang.plugins.runtime.metabolism import DynamicFBAConfig, DynamicFluxBalance
