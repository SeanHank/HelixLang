...
1. **Whole-cell-scale build** -- a synthetic genome (default 500 genes, the
   ...
   wired into a genome-wide :class:`~helixlang.plugins.runtime.grn.GRN` + :class:
   `~helixlang.plugins.runtime.virtual_cell.VirtualCell`.  A minimal gene-set is necessary
...
from helixlang.plugins.runtime.virtual_cell import VirtualCell, VirtualCellConfig, encode_gene
...
#: in :mod:`helixlang.plugins.runtime.metabolism` (the Phase-4 enzyme-capacity wiring);
...
def load_genome(source: str | dict[str, str],
    ...
    - a dict of ``gene -> protein`` (each protein is codon-optimized with
      :func:`~helixlang.plugins.runtime.virtual_cell.encode_gene`), or
    - FASTA text (``>gene_name`` headers, DNA per record) translated
      through the central-dogma pipeline and re-encoded with an RBS so
      the :class:`~helixlang.plugins.runtime.virtual_cell.VirtualCell` can transcribe it,
      or
...
def load_chromosome(fasta: str, gff: str) -> Chromosome:
    ...
    :class:`~helixlang.plugins.runtime.virtual_cell.VirtualCell` can transcribe them.  The
