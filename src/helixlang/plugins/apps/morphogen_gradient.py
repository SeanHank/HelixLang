...
- Diffusion ``D`` (physical µm^2/s, converted on-lattice via
  :func:`helixlang.core.units.diffusion_to_lattice`) + first-order decay
...
from helixlang.plugins.runtime.grn import GRN, sigmoid
...
def make_template_grn(genes: tuple[MorphogenGene, ...],
    ...
    :class:`~helixlang.plugins.runtime.population.CellPopulation`.
    """
    grn = GRN()
    for gene in genes:
...
class MorphogenGradient:
    ...
    def _diffuse_decay(self) -> None:
        ...
        :mod:`helixlang.plugins.runtime.population`), so the scheme never blows up.
        """
        cfg = self.config
        d = self._d_lattice()
        if d > 0.0:
