...
# NOTE: no Layer-2/plugins imports at module level.  The VM is part of the
# minimal core (doc/36 §2.2), so scientific modules it touches at runtime are
# pulled in lazily, per method, keeping ``import helixlang.core`` dependency-free.
if TYPE_CHECKING:
    from helixlang.plugins.runtime.lsystem import LSystem
    from helixlang.plugins.runtime.reaction_diffusion import GrayScott

# OP_FEED energy amount (ATP molecules). Defined here — rather than lazily
# within the dispatcher — so tests can monkeypatch it on this module; the value
# matches ``helixlang.plugins.runtime.cell.FEED_ENERGY_AMOUNT``.
FEED_ENERGY_AMOUNT = 1.0e8

# Runtime-opcode semantic constants (physical-unit registry) live in
# :mod:`helixlang.core.opcode_semantics` (imported above) so the Layer-2
# biological runtime (``plugins/runtime/population``) and the VM share them
# without a module-load-time coupling.
...
def _pop_op(vm: CellVM, n: int) -> list:
    """Pop ``n`` operand values, raising ``StackUnderflowError`` if absent.

    Replaces the prototype's silent no-op-on-underflow (doc/36 F11): a
    degenerate program must fail loudly, not silently compute a wrong result.
    """
    if len(vm.stack) < n:
        raise StackUnderflowError(
            f"operand stack underflow: needed {n}, had {len(vm.stack)}")
    return [vm.stack.pop() for _ in range(n)]
...
class BioInstructionDispatcher:
    ...
    def dispatch(self, op: Op) -> None:
        ...
        match op:
            ...
            case Op.OP_USE_PLUGIN:
                # doc/36 §3.2: activate a plugin backend through the registry.
                # Idempotent (a later use of the same plugin is a cache hit).
                idx = vm._read_u8()
                spec = vm.chunk.constants[idx] if idx < len(vm.chunk.constants) \
                    else None
                if spec is not None and spec[0] == "use_plugin":
                    vm._use_plugin(*spec[1:])
            ...
            case Op.OP_POP:
                _pop_op(vm, 1)
            case Op.OP_DUP:
                # duplicate top: pop it, push the original value twice
                v = _pop_op(vm, 1)[0]
                vm.stack.append(v)
                vm.stack.append(v)
            case Op.OP_SWAP:
                # _pop_op returns [top, second]; push top then second -> [x,y] -> [y,x]
                t, s = _pop_op(vm, 2)
                vm.stack.append(t)
                vm.stack.append(s)
            ...
            case Op.OP_WRITE_MEM:
                slot = vm._read_u8()
                vm.cell.slots[slot] = _pop_op(vm, 1)[0]
    ...
    def _handle_histone(self, inst: BioInstruction) -> None:
        ...
        from helixlang.plugins.runtime.epigenetics import HISTONE_MARK_TYPES
        vm = self._vm
        target = inst.target
    ...
    def _handle_quorum(self, inst: BioInstruction) -> None:
        ...
        from helixlang.plugins.runtime.population import QUORUM_SIGNAL_THRESHOLD
        vm = self._vm
        threshold = float(
...
class CellVM:
    ...
    def __init__(self, chunk: Chunk, program: Program, *, registry: Any = None):
        ...
        # doc/36 §3.2: activate every `use`-declared plugin before execution so
        # backends are present regardless of how genes are entered. Idempotent:
        # re-activating (e.g. via an OP_USE_PLUGIN op) is a cache hit.
        for d in program.use_directives:
            self._use_plugin(d.plugin, tuple(d.flags))

    def _use_plugin(self, plugin: str, flags: tuple[str, ...]) -> None:
        """Resolve/activate a plugin named by ``use`` through the registry.

        Capability flags are declared first (explicit opt-ins, doc/36 §3ξ.3);
        a missing or unsatisfied plugin raises the registry's explicit errors.
        """
        from helixlang.core.plugin_registry import Registry, get_registry
        registry: Registry = self._registry if self._registry is not None \
            else get_registry()
        for flag in flags:
            ...
            if self.program.config.use_central_dogma:
                ...
                if self._gem_dirty and self._metabolic_model is not None:
                    ...
                    try:
                        ...
                        # A GEM re-solve that fails silently leaves a stale
                        # FBA growth rate behind (doc/36 §3ξ.3).  Surface it
                        # unless reduced fidelity (Monod) is explicitly opted in.
    ...
    def _transcribe_translate(self) -> None:
        ...
        """
        from helixlang.plugins.runtime.bio_data import get_species_trna
