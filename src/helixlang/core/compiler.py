...
from helixlang.core.ast_nodes import Codon, Gene, Program
...
class Compiler:
    ...
    def compile(self, program: Program) -> Chunk:
        chunk = Chunk()
        # Plugin opt-ins (`use <plugin> [--flag ...]`, doc/36 §3.2): the
        # compiler materialises an OP_USE_PLUGIN per directive at the head of
        # the chunk, so the VM activates each plugin's backend before any gene
        # ORF runs. Activation through the registry is idempotent (a cache hit),
        # so this is safe to re-emit.
        for d in program.use_directives:
            const_idx = chunk.add_constant(("use_plugin", d.plugin, tuple(d.flags)))
            chunk.emit(Op.OP_USE_PLUGIN, const_idx, line=d.line, codon_index=-1)
