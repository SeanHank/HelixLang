...
def _encode_chunk(chunk: Chunk, program: Program) -> bytes:
    w = _Writer()
    # doc/36 F9: the bytecode opcode ABI (OPCODE_VERSION) is embedded so a
    # precompiled Chunk can never be replayed under a mismatched opcode layout —
    # an ABI mismatch is a hard ABIVersionError at load, never a wrong-result run.
...
def _decode_chunk(data: bytes) -> Chunk:
    r = _Reader(data, "CHNK")
    # doc/36 F9: refuse to load a Chunk whose opcode ABI predates/mismatches the
    # running VM. A stored 0 means "legacy artifact with no ABI marker" — still
    # rejected, because we cannot know its opcode layout is compatible.
    abi = r.u32()
    if abi != OPCODE_VERSION:
        raise ABIVersionError(OPCODE_VERSION, abi)
    ncode = r.u32()
    if ncode > _MAX_COUNT:
