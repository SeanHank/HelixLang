...
class Op(IntEnum):
    ...
    OP_RETURN = 0x12
    OP_NOP = 0x13
    # Plugin opt-in (doc/36 §3.2): compiler-generated, not mapped from a codon.
    # Its operand is a constant-pool index naming the plugin; the VM activates it
    # through the registry (idempotent, so repeated/program-level emits are safe).
    OP_USE_PLUGIN = 0x14
...
OP_OPERAND_BYTES: dict[Op, int] = {
    Op.OP_START: 0, Op.OP_HALT: 0, Op.OP_RETURN: 0, Op.OP_NOP: 0, Op.OP_USE_PLUGIN: 1,
