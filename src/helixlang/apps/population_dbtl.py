...
:mod:`helixlang.apps.protein_evolution`):
...
- **Build** — the designed DNA is assembled into an ecosystem
  :class:`~helixlang.apps.ecosystem.Species`; the DNA sequence decodes
...
- **Test** — each candidate is grown in a sealed chemostat
  (:class:`~helixlang.apps.ecosystem.Ecosystem`) and its long-term
  specific growth rate (invasion-fitness proxy) is measured.
- **Learn** — :func:`~helixlang.virtual_cell.fit_parameters` calibrates
...
def learn_surrogate(genomes: list[str], growths: list[float],
    ...
    :func:`~helixlang.virtual_cell.fit_parameters` against the observed
