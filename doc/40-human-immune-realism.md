...
> **Status:** plan (literature survey complete, pending implementation) · 2026-08-30 · baseline 2026.8.5
>
...
> model specified in doc/31 — organized per the requested structure **内容 (content) /
> 方法 (method) / 结果 (results) / 结论 (conclusions)** of each reference work, with every
...
## 2 — 内容 (Content) Audit: What `immune.py` Actually Is
...
|---|---|---|---|
| G1 | Type I IFN (IFN-α/β) antiviral loop | missing | viral kinetics realism (flu bursts, HSV) |
...
| G4 | Bone-marrow granulopoiesis / neutropenia | linear neutrophil production + floor `:126-149` | chemo myelosuppression plots (doc/32), recovery kinetics |
...
| G8 | Acute-phase panel (CRP, SAA, ferritin, PCT, fibrinogen) | CRP only, linear | sepsis severity scoring (PCT), chronic disease |
| G9 | IL-6/CRP kinetics fidelity (lag, sigmoid, wide dynamic range) | linear, capped 200 mg/L | vs Sproston/Ashworth ranges (<1→1000 µg/ml) |
...
| G11 | Circadian HPA/cortisol coupling (time-resolved) | static scalar cortisol suppression `:235-239` | stress/inflammation daily rhythm |
...
## 4 — Literature Evidence Base, by 内容/方法/结果/结论

The project's own prior surveys (doc/31 §2.4 table) plus primary literature. Each entry
records **内容** (systems modelled), **方法** (formulation used by the authors), **结果**
(quantitative outcomes), **结论** (what our plan adopts or rejects).

### L1 — Basic Immune Simulator (BIS), Folcik/An/Orosz, *Theor. Biol. Med. Model.* 4:39 (2007, 8:1 2011)

- **内容:** innate (macrophage, neutrophil, DC, mast, eosinophil, NK) + adaptive
  (B, Th, Tc, Tr; IgG/IgM/IgA/IgE; complement) agents moving through blood/tissue/lymph.
- **方法:** agent-based (RepastJ), cell-as-agent, diffusing cytokines; rules from ~146 papers.
- **结果:** reproduces qualitatively correct immune timeline (innate peak → adaptive peak →
  resolution), memory anamnesis, dose-dependence of infection outcomes.
- **结论:** the *rule skeleton* is implementable (doc/31 `:102`), but full spatial ABM is a
...
- **内容:** macrophage/neutrophil/tissue-cytokine network for sepsis; prior-knowledge rules.
- **方法:** ABM; Cockrell 2021 adds **GA calibration of 432 continuous free parameters** to fit
  burn-patient cytokine time series.
- **结果:** An 2004 qualitatively reproduced the *failed* anti-TNF and anti-IL-1 sepsis trials
  (networks compensate); calibrated IIRABM recovers clinical cytokine trajectories.
- **结论:** (a) cytokine networks are **redundant/counterbalanced** — no cure from blocking one
...
- **内容:** P/DAMP + pro-inflammatory (P) + anti-inflammatory (A) mediator balance.
- **方法:** minimal 3–4 ODEs, bistability, dimensional analysis; part II adds tolerance/potentiation.
- **结果:** two stable phenotypes (resolution vs persistence) emerge from *parameter* changes;
  explains chronic inflammation and sepsis death vs survival, and drug-effect paradoxes.
- **结论:** an ODE core can exhibit realistic nonlinear behavior **without ABM**. This is the
...
- **内容:** proliferating → maturation transit compartments (with mean transit time) →
...
- **方法:** ODE compartment chain (4–5 transit pools), feedback Hill; fit to clinical ANC nadir
  curves across many chemo drugs with consistent parameters.
- **结果:** gold-standard reproduction of delayed onset, ANC nadir ≈7–10 days, recovery
  overshoot; parameter consistency across drugs.
- **结论:** **G4 fix.** Replaces the linear production/floor in `ImmuneCellPopulation` (`:126-149`)
...
- **内容:** target-cell epithelial infection + **type I IFN** + innate (NK-like) + **CD4/CD8 T**
  + **antibody** (dual effector arms).
- **方法:** ODEs, parameterized from mouse influenza; effectors act with time delays/loss.
- **结果:** early viral decline driven by IFN/innate arm, late clearance by CD8 effectors;
  antibody matters for recurrence/reinfection; predicts which arm dominates per scenario.
- **结论:** a *complete but compact* immune equation set exists and is validated — use it as the
...
- **内容:** within-host COVID: virus, IFN, CD8 activation (log-linear), innate immune;
  virtual population from parameter sampling.
- **方法:** QSP ODE + population sampling fitting real patient trajectories; log-linear T-cell
  activation avoids sharp thresholds.
- **结果:** population-level heterogeneity explains individual courses; intervention
  (interferon timing) effects differ by virtual-patient subgroup.
- **结论:** the **virtual-population methodology (G13)** — sample parameters, keep log-linear
...
- **内容:** C3/C5 convertase networks; alternative + classical/lectin; in vivo plasma+surface.
- **方法:** large ODE/Gillespie networks (tens–hundreds of states, 142+ rate constants);
  inhibition with compstatin/eculizumab-type agents tested *in silico*.
- **结果:** predicts C3 fragment deposition, MAC formation thresholds, and inhibitor dose-effect
  curves consistent with assays.
- **结论:** full network is overkill (state explosion); adopt a **reduced 6–10 ODE complement
...
- **内容:** two-dose vaccine schedule → memory B + long-lived plasma cells (LLPC), IgM→IgG
  class switching, waning.
- **方法:** consensus linear ODE (chain-trick compartment model), fit across vaccines
  (incl. mRNA), Age/adjuvant terms.
- **结果:** resembles observed sigmoid rise → peak ~2–4 wks → biphasic waning (short vs long-lived
  plasma); second dose boost amplitude ruled by memory-to-PC conversion.
- **结论:** **G2/G3/G7/G12 unified formulation** for the vaccination feature: an 8–10 compartment
...
- **内容:** IL-6-driven hepatic CRP; broad dynamic range **<1 → 1000 µg/ml**; ~6 h synthesis
  onset after stimulus, ~19 h half-life; CRP is delayed/compensatory, also IL-1β/IL-17.
- **方法:** clinical measurements + genotype/kinetics analysis.
- **结果:** CRP peak lags IL-6 by ~18–48 h; very high ranges (>300 mg/L) occur in severe sepsis;
  chronic low-level elevation tracks atherosclerosis/aging.
- **结论:** **G8/G9 fix.** Current `CRPDriver` linear-IL6→linear-CRP with hard cap 200 and no
...
- **内容:** tocilizumab/siltuximab PD on IL-6→CRP, with ~10% lymph-node/tissue penetration.
- **方法:** target-mediated drug disposition + IL-6/CRP circuit QSP, calibrated to sIL-6R.
- **结果:** reproduces CRP drop kinetics under anti-IL-6 and rebound on washout.
- **结论:** our existing `anti_inflammatory`/`immunosuppression` pathway (`:250-253`) should
...
- **内容:** cell-virus-IFN circuit; paracrine IFN-α/β establishes antiviral state with
  characteristic biphasic/multi-staged viral curves.
- **方法:** within-host ODE/stochastic with IFN autocrine-paracrine loop, delay, priming.
- **结果:** predicts early exponential viral rise slowed/aborted by IFN, and tissue-dependent
  spread restriction.
- **结论:** supplies the **EGF-ish autocrine shape** for the G1 component with physiologically
...
- **内容:** consensus that acute inflammation is a **dynamical system with redundancies**,
  resolvable at ODE *and* ABM granularity; influenza-control lessons on nonlinearity.
- **方法/结果/结论:** hybrid ODE+ABM tiers are the community norm; nonlinear (threshold/Hill)
...
## 5 — 内容/方法/结果/结论 Synthesis: From Literature to Implementation

Mapping each gap of §3 to its evidence and to a concrete planned intervention. The four columns
answer: **内容** — what we will build; **方法** — modeling method from the cited paper(s);
**结果** — expected quantitative behavior; **结论** — the validation gate proving it.

| Gap | 内容 (build) | 方法 (from) | 结果 (expected) | 结论 (gate) |
|---|---|---|---|---|
